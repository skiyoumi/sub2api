export default {
  ...
  keys: {
    ...
    ccsImport: {
      ...
      noModels: '当前密钥没有可用模型',
      open: '打开 CC Switch',
      openRequested: '已请求打开 CC-Switch。请允许浏览器打开应用，并切换到 CC-Switch 确认导入。若无响应，请检查 CC-Switch 是否已安装，或手动复制 API 密钥配置。',
      openFailed: '无法打开 CC-Switch。请检查浏览器是否允许打开外部应用，或手动复制 API 密钥配置。'
    },
    // 配额和有效期
    quotaLimit: '额度限制',
