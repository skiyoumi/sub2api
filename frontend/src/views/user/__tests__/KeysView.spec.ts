...
const SelectStub = {
  name: 'Select',
  methods: { clearSearch: vi.fn() },
  props: ['modelValue', 'options'],
  emits: ['update:modelValue'],
  template: '<div><slot name="dropdown-header" /><select :value="modelValue" @change="$emit(\'update:modelValue\', $event.target.value)"></select></div>',
}
...
describe('user KeysView', () => {
  ...
  describe('create provider selection', () => {
    ...
    const optionIds = (wrapper: VueWrapper) => groupSelect(wrapper).props('options')
      .filter((option: { kind?: string }) => option.kind !== 'group')
    ...
    it('combines provider selection with platform tabs and clears a stale platform on provider change', async () => {
      const wrapper = await openCreate()
      await chooseProvider(wrapper, 'domestic')
      await getButtonByText(wrapper, 'Kimi').trigger('click')
      expect(optionIds(wrapper)).toEqual([3])
      groupSelect(wrapper).vm.$emit('update:modelValue', 3)
      await nextTick()
      await chooseProvider(wrapper, 'other')
      expect(optionIds(wrapper)).toEqual([7, 8, 9, 10, 11])
      expect(groupSelect(wrapper).props('modelValue')).toBeNull()
      expect(groupSelect(wrapper).findAll('button').some(button => button.text() === 'Kimi')).toBe(false)
    })
