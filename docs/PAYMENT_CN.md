...
进入 **用户管理 → 编辑用户**，开启 **禁止充值套餐赠送** 并保存。该用户仍可购买充值套餐，但新订单仅获得原有充值倍率计算的常规到账金额，套餐赠送为 0；充值页面也不再展示赠送金额。关闭开关后恢复赠送资格。

此设置仅影响保存后新建的订单，已创建订单继续按下单时的金额到账。已有余额（含未过期的赠送余额）、后台手动赠送、自定义金额充值和订阅购买不受影响。升级时需应用 `239_user_recharge_bonus_disabled.sql` 迁移，所有用户默认仍享受赠送。
