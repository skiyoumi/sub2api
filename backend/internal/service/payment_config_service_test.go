...
func TestUpdatePaymentConfig_OmittedRechargePackageSettingsArePreserved(t *testing.T) {
	wantRechargeSettings := map[string]string{
		SettingRechargePackagesEnabled: "true",
		SettingAllowCustomRecharge:     "false",
		SettingRechargePackages:        `[{"id":"pkg_10","amount":"10.00","bonus_amount":"0","enabled":true}]`,
	}
	initial := make(map[string]string, len(wantRechargeSettings))
	for key, value := range wantRechargeSettings {
		initial[key] = value
	}
	repo := &paymentConfigSettingRepoStub{values: initial}
	svc := &PaymentConfigService{settingRepo: repo}
	...
	if len(repo.updates) != 1 || repo.updates[SettingPaymentEnabled] != "true" {
		t.Fatalf("updates = %v, want only payment enabled", repo.updates)
	}
	for key, value := range wantRechargeSettings {
		if repo.values[key] != value {
			t.Fatalf("recharge setting %q = %q, want preserved value %q", key, repo.values[key], value)
		}
	}
}

func TestUpdatePaymentConfig_PersistsExplicitRechargePackageSettings(t *testing.T) {
	repo := &paymentConfigSettingRepoStub{values: map[string]string{
		SettingRechargePackagesEnabled: "true",
		SettingAllowCustomRecharge:     "true",
		SettingRechargePackages:        `[{"id":"pkg_10","amount":"10.00","bonus_amount":"0","enabled":true}]`,
	}}
	svc := &PaymentConfigService{settingRepo: repo}

	falseValue := false
	emptyPackages := []RechargePackage{}
	err := svc.UpdatePaymentConfig(context.Background(), UpdatePaymentConfigRequest{
		RechargePackagesEnabled: &falseValue,
		AllowCustomRecharge:     &falseValue,
		RechargePackages:        &emptyPackages,
	})
	if err != nil {
		t.Fatalf("UpdatePaymentConfig returned error: %v", err)
	}
	...
	want := map[string]string{
		SettingRechargePackagesEnabled: "false",
		SettingAllowCustomRecharge:     "false",
		SettingRechargePackages:        "[]",
	}
	if len(repo.updates) != len(want) {
		t.Fatalf("updates = %v, want exactly %v", repo.updates, want)
	}
